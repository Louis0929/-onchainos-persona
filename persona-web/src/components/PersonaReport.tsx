import type { PersonaReport as PersonaReportType, PersonaArchetype } from "../types";

const ARCHETYPE_EMOJI: Record<PersonaArchetype, string> = {
  whale: "🐋", degen: "🎰", diamond_hands: "💎", paper_hands: "🧻",
  market_maker: "🏭", gambler: "🎲", farmer: "🌾", project_dev: "👨‍💻",
  retail: "🛒", launderer: "🌀", hodler: "🤲", bridge_hopper: "🌉",
};

const ARCHETYPE_LABEL: Record<PersonaArchetype, string> = {
  whale: "大鯨魚", degen: "Degen", diamond_hands: "鑽石手", paper_hands: "紙手",
  market_maker: "做市商", gambler: "賭徒", farmer: "農夫", project_dev: "項目方",
  retail: "散戶", launderer: "洗錢嫌疑", hodler: "佛系持幣", bridge_hopper: "跨鏈玩家",
};
...
export default function PersonaReport({ report }: { report: PersonaReportType }) {
  const arch = report.personality.archetype;
    ...
    <div className="mt-8 space-y-6">
      ...
      <div className="cyber-border rounded-lg p-6 bg-cyber-panel">
        <div className="flex items-center gap-4 mb-4">
          <span className="text-5xl">{ARCHETYPE_EMOJI[arch]}</span>
          <div>
            <h2 className="text-2xl font-bold text-cyber-accent">{ARCHETYPE_LABEL[arch]}</h2>
            <p className="text-sm text-gray-400 font-mono">{report.address.slice(0, 10)}...{report.address.slice(-8)}</p>
          </div>
          ...
          <div className="mt-3 flex gap-2 flex-wrap">
            {report.personality.secondaryArchetypes.map(a => (
              <span key={a} className="text-xs border border-cyber-border rounded px-2 py-0.5 text-gray-400">
                {ARCHETYPE_EMOJI[a]} {ARCHETYPE_LABEL[a]}
              </span>
            ))}
          </div>
        )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="cyber-border rounded-lg p-6 bg-cyber-panel">
          ...
          <div className="text-center mb-4">
            <div className="text-4xl font-bold" style={{ color: trustColor }}>{ts.score}</div>
          ...
          <TrustBar value={ts.breakdown.authenticity} label="真實性" color="#00f0ff" />
